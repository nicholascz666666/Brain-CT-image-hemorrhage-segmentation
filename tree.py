...
data = pd.read_csv(r"mixed_windows.csv")
x = data.iloc[:,:-1]
y = data.iloc[:,-1]
x_train,x_test,y_train,y_test=train_test_split(x,y,test_size=0.20,random_state=77,stratify=y)
f = open("demofile2.txt", "a")
f.write("model for devision tree")
#devision tree
dtc=DecisionTreeClassifier()
dtc = dtc.fit(x_train,y_train)
y_test_pred = dtc.predict(x_test)
f.write("accuracy for testing is {}".format(accuracy_score(y_test,y_test_pred)))
...
f.write(pd.DateFrame(table_test))
...
f.write(pd.DateFrame(table_test_ada))  
f.close()
